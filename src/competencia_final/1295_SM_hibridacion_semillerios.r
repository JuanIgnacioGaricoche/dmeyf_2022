...
PARAM <- list()
PARAM$experimento <- "ZZ9999_hibridacion_semillerio_kaggle_mfinal"
...
tb_prediccion_hibridador <- data.table(
  ...
  prediccion = rowMeans(tb_hibridador[, c(-1)]) # excluye el numero_de_cliente del c??lculo de la media
)
